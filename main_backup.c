...
volatile bool uart_rx_seen = false; // set true on first RX to stop UART heartbeat prints
...
void GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(HEARTBEAT_LED_PORT, HEARTBEAT_LED_PIN, GPIO_PIN_RESET);
    ...
    /* Quick LED self-test: blink both LEDs briefly to verify wiring (respects active-low) */
...
void USART1_IRQHandler(void)
{
    ...
    uart_rx_seen = true; // mark that UART RX is alive
...
int main(void)
{
  ...
  USART1_Init();
    ...
    /* Print a short boot banner to confirm UART TX and firmware features */
    UART_SendCRLF();
    UART_SendString("BOOT: MX1508 Motor Controller v2.1 (STM32F401RC)\r\n");
    ...
    UART_SendString("OFF\r\n");
#endif
    UART_SendString("Pins: TRIG A=PB0, B=PB1 | ECHO A=PB6, B=PB7 | LEDs L=PB14 R=PB15\r\n");
#endif
...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
    ...
    HAL_GPIO_TogglePin(HEARTBEAT_LED_PORT, HEARTBEAT_LED_PIN);
        ...
        if (!uart_rx_seen) {
                if ((hb_counter++ % 4U) == 0U) { // ~1s cadence with 250ms loop
                        UART_SendString("HB t="); UART_SendUInt(HAL_GetTick()); UART_SendCRLF();
                }
        }
#endif
        osDelay(250);
...
void StartUltrasonicTask(void const * argument)
{
    ...
    /* Small gap to avoid cross-talk between sensors */
    osDelay(5);
    uint16_t right = Ultrasonic_MeasureB();
        ultrasonic_left_cm = left;
        ultrasonic_right_cm = right;

        /* Drive debug LEDs: ON when within slow/stop zone, OFF otherwise */
...
#if WALL_LED_ACTIVE_LOW
    left_led = (left_led == GPIO_PIN_SET) ? GPIO_PIN_RESET : GPIO_PIN_SET;
    right_led = (right_led == GPIO_PIN_SET) ? GPIO_PIN_RESET : GPIO_PIN_SET;
#endif
        HAL_GPIO_WritePin(WALL_LED_PORT, WALL_LEFT_LED_PIN, left_led);
        HAL_GPIO_WritePin(WALL_LED_PORT, WALL_RIGHT_LED_PIN, right_led);

#if ULTRASONIC_DEBUG
    /* Every ~10 cycles (~500ms), print diagnostics */
    if ((dbg_counter++ % 10U) == 0U)
    {
        UART_SendString("US L="); UART_SendUInt(left);
        UART_SendString("cm R="); UART_SendUInt(right);
        UART_SendString("cm LEDL="); UART_SendUInt((uint32_t)(left_led == GPIO_PIN_SET));
        UART_SendString(" LEDR="); UART_SendUInt((uint32_t)(right_led == GPIO_PIN_SET));
            UART_SendString(" CMD=");
            while (!(USART1->SR & USART_SR_TXE)) { }
            USART1->DR = (uint8_t)last_movement_cmd; /* quick single-char print */
            UART_SendCRLF();
    }
#endif
    ...
    /* Center-seeking steering only when moving forward */
        ...
        if (left_ok && right_ok)
        {
            ...
            /* anti-windup: limit integral contribution (use runtime check; compiler will fold constants) */
